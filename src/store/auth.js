...
const meURL = `${__API_INVESTIGACION__}/inves/me`; // URL para el endpoint de login
...
apiClient.interceptors.request.use(
  (config) => {
    const token = localStorage.getItem('token_inves');
    const tokenType = localStorage.getItem('token_type_inves');
    if (token && tokenType) {
...
export const getMe = async () => {
  try {
    ...
    localStorage.setItem('user_inves', JSON.stringify(response.data));
    logged.value = true;
